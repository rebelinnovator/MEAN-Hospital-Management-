...
import { GetServiceUserMedHistory, IllnessList, IllnessData, MedicalHistory, ApiResponse } from '../../client.interface';

// Component
import { ClientProfileLayoutComponent } from '../client-profile-layout/client-profile-layout.component';

@Component({
  selector: 'app-service-user-medical-history',
  templateUrl: './service-user-medical-history.component.html'
})
export class ServiceUserMedicalHistoryComponent implements OnInit {
  ...
  setIllness() {
    ...
    if (illness && illness.length) {
      this.illnessList = this.illnessList.map((ill: any) => {
    ...
    this.clientService.addUpdateServiceUserMedicalHistory(this.regularForm.value).subscribe(
      (response: ApiResponse) => {
        ...
        if (status) {
          ...
          this.layout.changeDropDownValueFromComponent('Referral');
